...
before_save :default_values
 belongs_to :user
  ...
  validates_attachment :photo , :content_type => { :content_type => ["image/jpeg", "image/gif", "image/png"] }, :size => { :in => 0..5000.kilobytes, :message => "Photo must be less than 5 megabytes in size"}
  ...
  validates_attachment :gradebook_photo , :content_type => { :content_type => ["image/jpeg", "image/gif", "image/png"] }, :size => { :in => 0..5000.kilobytes, :message => "Photo must be less than 5 megabytes in size"}

  has_attached_file :toefl, :styles => { :medium => "250x250", :thumb => "100x100" }, :default_url => ":style/missing.jpg"
  validates_attachment_content_type :toefl, :content_type => /\Aimage\/.*\Z/
  validates_attachment :toefl , :content_type => { :content_type => ["image/jpeg", "image/gif", "image/png"] }, :size => { :in => 0..5000.kilobytes, :message => "Photo must be less than 5 megabytes in size"}
  ...
  validates :school_region, :school_town, :on => :update, format: { with: /\A[a-zA-Z ]+\z/, message: "allows only letters" }, :if => :active_or_educational?
